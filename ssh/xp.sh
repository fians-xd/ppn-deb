...
do
    ...
    if [[ "$exp2" -le "0" ]]; then
        # Menghapus entri dengan atau tanpa tanda # di depan, hingga baris yang mengandung "email"
        sed -i "/^### $user $exp/,/\"email\": \"$user\"/d" /etc/xray/config.json
        sed -i "/^# ### $user $exp/,/\"email\": \"$user\"/d" /etc/xray/config.json
...
do
    ...
    if [[ "$exp2" -le "0" ]]; then
        # Menghapus entri dengan atau tanpa tanda # di depan, hingga baris yang mengandung "email": "user"
        sed -i "/^#! $user $exp/,/\"email\": \"$user\"/d" /etc/xray/config.json
        sed -i "/^# #! $user $exp/,/\"email\": \"$user\"/d" /etc/xray/config.json
    fi
done
